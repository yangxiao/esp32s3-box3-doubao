...
/* Parsed response */
typedef struct {
    int message_type;       /* MSG_SERVER_FULL_RESPONSE, MSG_SERVER_ACK, MSG_SERVER_ERROR */
    int event;              /* event code, -1 if none */
    int has_event;
    uint32_t seq;
    int has_seq;
    char session_id[128];
    uint32_t payload_size;
    uint8_t *payload_data;  /* decompressed + deserialized data (caller must free) */
    size_t payload_data_len;
    int is_binary;          /* 1 if payload is raw audio, 0 if JSON text */
    uint32_t error_code;
} parsed_response_t;

/*
 * Generate protocol header (4 bytes).
 ...
 * resp->payload_data is allocated with malloc, caller must free.
